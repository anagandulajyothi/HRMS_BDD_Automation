...
class CucumberJSAllureFormatter extends cucumber_1.Formatter {
    ...
    onTestCaseStarted(data) {
        ...
        if (!((_c = scenario === null || scenario === void 0 ? void 0 : scenario.examples) === null || _c === void 0 ? void 0 : _c.length)) {
            return;
        }
    ...
    handleAllureAttachment(payload) {
        const { labels = [], links = [], parameter = [], categories = [], step, description, descriptionHtml, environmentInfo, } = payload.metadata;
        if (links.length > 0) {
    ...
    handleAllureStep(payload) {
        const _a = payload.metadata, { attachments: metadataAttachments } = _a, metadata = __rest(_a, ["attachments"]);
        const attachments = metadataAttachments.map(({ name, type, content }) => {
        ...
        const testStep = Object.assign(Object.assign({}, metadata), { attachments, steps: [] });
        if (payload.step) {
